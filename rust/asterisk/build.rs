...
fn main() {
    ...
    let bindings = bindgen::Builder::default()
        ...
        .clang_arg("-fblocks")
        .allowlist_function("ast_(audiohook_.*|channel_.*|cli|config_.*|datastore_.*|dsp_.*|format_.*|frame_free|hangup|jb_.*|log|module_.*|moh_.*|pthread_.*|queue_frame|read_textfile|sem_.*|setstate|taskprocessor_.*|verbose)")
        ...
        .allowlist_type("(ao2_lock_req|ast_(assigned_ids|audiohook|audiohook_direction|channel|channel_iterator|channel_tech|cli_args|cli_entry|control_frame_type|datastore|datastore_info|dsp|format|format_cap|frame|jb_conf|module|sem|taskprocessor))")
