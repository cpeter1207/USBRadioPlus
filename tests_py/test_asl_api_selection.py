...
def detection_environment():
    ...
    for inherited_override in (
            "ASL_RADIO_API", "MAKEFLAGS", "MFLAGS", "MAKEOVERRIDES"):
...
def selected_api(include_dir: Path) -> str:
    return subprocess.run(
        ["make", "-s", "print-asl-radio-api",
         f"ASTERISK_INCLUDEDIR={include_dir}"],
        cwd=ROOT,
        env=detection_environment(),
        check=True,
        text=True,
...
def test_legacy_headers_select_oss_backend():
    database = make_database(
        ROOT / "tests/fixtures/asterisk-dev/include")
...
def test_shared_device_headers_select_modern_backend():
    database = make_database(
        ROOT / "tests/fixtures/asterisk-modern/include")
