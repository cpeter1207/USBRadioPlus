...
DIST_TOP := Makefile VERSION COPYING README.md INSTALL.md RELEASE-CHECKLIST.md install.sh
...
.PHONY: all check clean dist distcheck install install-strip install-from-dist uninstall
...
$(MODULE): $(MODULE_SOURCES) | $(BUILD_DIR)
	$(CC) $(CPPFLAGS) $(COMMON_CPPFLAGS) $(DSP_CFLAGS) $(CFLAGS) $(WARNFLAGS) \
		-fPIC -DAST_MODULE='"chan_usbradioplus"' \
		-DAST_MODULE_SELF_SYM=__internal_chan_usbradioplus_self \
		-shared $(LDFLAGS) -o $@ src/chan_usbradioplus.c $(DSP_LIBS) -lusb -lm
