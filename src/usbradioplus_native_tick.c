...
void usbradioplus_native_tick(struct chan_usbradio_pvt *o)
{
	...
	if (o->plus_advanced) {
		/* Asterisk and CM119 clocks remain independent even when both advertise
		 * 48 kHz. The persistent converter makes that correction continuously. */
		if (!read_native_program_elastic(o)) {
			if (o->txkeyed)
