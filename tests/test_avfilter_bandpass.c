...
/** @brief Measure the requested brick-wall filter response.
 * @param frequency Test frequency in Hz.
 * @param post_limiter Nonzero selects the post-limiter band-pass.
 * @param highpass Lower edge of the post-limiter band-pass in Hz.
 * @return Measured level or response used by the caller's numerical assertions.
 */
static double measure(double frequency, int post_limiter, double highpass)
{
	...
	if (post_limiter) {
		...
		config.post_limiter_bandpass_highpass_hz = highpass;
		config.post_limiter_bandpass_lowpass_hz = 3000.0;
	} else {
...
int main(void)
{
	double low = measure(100.0, 0, 0.0);
	double pass = measure(1000.0, 0, 0.0);
	double high = measure(5000.0, 0, 0.0);
	double post_low = measure(100.0, 1, 300.0);
	double post_pass = measure(1000.0, 1, 300.0);
	double post_high = measure(5000.0, 1, 300.0);
	double no_lower_edge = measure(100.0, 1, 0.0);
	double low_rejection = 20.0 * log10(pass / low);
	double high_rejection = 20.0 * log10(pass / high);
	double post_low_rejection = 20.0 * log10(post_pass / post_low);
	double post_high_rejection = 20.0 * log10(post_pass / post_high);
	...
	if (low_rejection < 60.0 || high_rejection < 60.0)
		return 2;
	if (!isfinite(post_pass) || post_pass < 650.0 || post_pass > 750.0)
		return 3;
	if (post_low_rejection < 60.0 || post_high_rejection < 60.0)
		return 4;
	if (!isfinite(no_lower_edge) || no_lower_edge < 650.0 || no_lower_edge > 750.0)
		return 5;
	return 0;
}
